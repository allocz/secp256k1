...
func (p *PublicKey) FromBytes32(data []byte) *PublicKey {
	...
	pubb[0] = 0x02
	copy(pubb[1:], data)
	ok := C.secp256k1_ec_pubkey_parse(ctx, &p.p,
		(*C.uchar)(unsafe.Pointer(&pubb)), C.size_t(33))
	if ok == 0 {
		return nil
	}
	return p
...
func (p *PublicKey) FromBytes64(data []byte) *PublicKey {
	...
	copy(pubb[1:], data)
	ok := C.secp256k1_ec_pubkey_parse(ctx, &p.p,
		(*C.uchar)(unsafe.Pointer(&pubb[0])), C.size_t(65))
	if ok == 0 {
		return nil
	}
	return p
...
func (p *PublicKey) toBytes32(data []byte) []byte {
	...
	dlen := 33
	C.secp256k1_ec_pubkey_serialize(ctx,
		(*C.uchar)(unsafe.Pointer(&pubb[0])),
		(*C.size_t)(unsafe.Pointer(&dlen)),
		&p.p, C.SECP256K1_EC_COMPRESSED)
	copy(data, pubb[1:])
	return data
}
...
func (p *PublicKey) toBytes64(data []byte) []byte {
	...
	C.secp256k1_ec_pubkey_serialize(ctx,
		(*C.uchar)(unsafe.Pointer(&pubb[0])),
		(*C.size_t)(unsafe.Pointer(&dlen)),
		&p.p, C.SECP256K1_EC_UNCOMPRESSED)
	copy(data, pubb[1:])
	return data
}
...
func (p *PublicKey) FromPrivateKey(priv *PrivateKey) *PublicKey {
	...
	ok := C.secp256k1_ec_pubkey_create(ctx, &p.p,
		(*C.uchar)(unsafe.Pointer(&priv.k[0])))
	if ok == 0 {
		return nil
	}
...
func (e *ECDSASignature) fromBytes64(data []byte) *ECDSASignature {
	...
	ok := C.secp256k1_ecdsa_signature_parse_compact(ctx, &e.s,
		(*C.uchar)(unsafe.Pointer(&data[0])))
	if ok == 0 {
		return nil
	}
...
func (e *ECDSASignature) toBytes64(data []byte) []byte {
	...
	C.secp256k1_ecdsa_signature_serialize_compact(ctx,
		(*C.uchar)(unsafe.Pointer(&data[0])), &e.s)
	return data
}
...
func (e *ECDSASignature) Sign(priv *PrivateKey, hash []byte) *ECDSASignature {
	...
	ok := C.secp256k1_ecdsa_sign(ctx, &e.s,
		(*C.uchar)(unsafe.Pointer(&hash[0])),
		(*C.uchar)(unsafe.Pointer(&priv.k[0])),
		C.secp256k1_nonce_function_rfc6979,
		unsafe.Pointer(nil))
...
func (e *ECDSASignature) Verify(pub *PublicKey, hash []byte) bool {
	...
	ok := C.secp256k1_ecdsa_verify(ctx, &e.s,
		(*C.uchar)(unsafe.Pointer(&hash[0])), &pub.p)
	return ok == 1
}
...
func SchnorrKeyPairFromBytes32(priv *PrivateKey, pub *PublicKey,
	...
	if priv.FromBytes32(privb) == nil {
		return fmt.Errorf("error parsing private key")
	}
	var pubb [64]byte
	if pub.FromPrivateKey(priv).ToBytes64(pubb[:]) == nil {
	...
	if pubb[63]&0x1 != 0 {
		...
		C.secp256k1_ec_seckey_negate(ctx,
			(*C.uchar)(unsafe.Pointer(&priv.k[0])))
		if pub.FromPrivateKey(&priv).ToBytes64(pubb[:]) == nil {
			return fmt.Errorf("error deriving public key from negated private key")
		}
	}
	return nil
}

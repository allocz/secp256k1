...
// mustFieldValInternal converts the passed hex string into a [FieldVal64] and
...
// values.
func mustFieldValInternal(s string, allowOverflow bool) *FieldVal64 {
	if len(s)%2 != 0 {
		s = "0" + s
	}
...
// mustFieldVal converts the passed hex string into a [FieldVal64] and will panic
// if there is an error.  Values that overflow are treated as an error.
//
// This is only provided for the hard-coded constants so errors in the source
// code can be detected. It will only (and must only) be called with hard-coded
// values.
func mustFieldVal(s string) *FieldVal64 {
...
// coordinates.
func MakeJacobianPoint(x, y, z *FieldVal64) JacobianPoint {
	var p JacobianPoint
	p.X.Set(x)
	p.Y.Set(y)
	p.Z.Set(z)
...
func addZ1AndZ2EqualsOne(p1, p2, result *JacobianPoint) {
	...
	// breakdown above.
	var h, i, j, r, v FieldVal64
	var negJ, neg2V, negX3 FieldVal64
...
func addZ1EqualsZ2(p1, p2, result *JacobianPoint) {
	...
	// Calculate X3, Y3, and Z3 according to the intermediate elements
	// breakdown above.
	var a, b, c, d, e, f FieldVal64
	var negX1, negY1, negE, negX3 FieldVal64
...
func addZ2EqualsOne(p1, p2, result *JacobianPoint) {
	...
	// Calculate X3, Y3, and Z3 according to the intermediate elements
	// breakdown above.
	var h, hh, i, j, r, rr, v FieldVal64
	var negX1, negY1, negX3 FieldVal64
	negX1.Set(x1).Negate(1)               // negX1 = -X1 (mag: 2)
...
func addGeneric(p1, p2, result *JacobianPoint) {
	...
	// terms.
	var z1z1, z2z2, u1, u2, s1, s2 FieldVal64
	z1z1.SquareVal(z1)                        // Z1Z1 = Z1^2 (mag: 1)
	...
	// Calculate X3, Y3, and Z3 according to the intermediate elements
	// breakdown above.
	var h, i, j, r, rr, v FieldVal64
	var negU1, negS1, negX3 FieldVal64
...
func isOnCurve(fx, fy *FieldVal64) bool {
	// Elliptic curve equation for secp256k1 is: y^2 = x^3 + 7
	y2 := new(FieldVal64).SquareVal(fy).Normalize()
	result := new(FieldVal64).SquareVal(fx).Mul(fx).AddInt(7).Normalize()
...
func DecompressY(x *FieldVal64, odd bool, resultY *FieldVal64) bool {
	...
	// the curve.
	x3PlusB := new(FieldVal64).SquareVal(x).Mul(x).AddInt(7)

...
import (
	...
	"github.com/allocz/secp256k1/internal/dcrsecpamd64"
)
...
var (
	...
	// times.
	orderAsFieldVal = func() secp256k1.FieldVal64 {
		var f secp256k1.FieldVal64
...
func fieldToModNScalar(v *secp256k1.FieldVal64) (secp256k1.ModNScalar, uint32) {
	var buf [32]byte
	v.PutBytes(&buf)
...
func modNScalarToField(v *secp256k1.ModNScalar) secp256k1.FieldVal64 {
	var buf [32]byte
	v.PutBytes(&buf)
	var fv secp256k1.FieldVal64
	fv.SetBytes(&buf)
	return fv
}
...
func (sig *Signature) Verify(hash []byte, pubKey *secp256k1.PublicKey) bool {
	...
	z := new(secp256k1.FieldVal64).SquareVal(&X.Z)

	// Step 8.
	//
	// Verified if R * z == X.x (mod P)
	sigRModP := modNScalarToField(&sig.r)
	result := new(secp256k1.FieldVal64).Mul2(&sigRModP, z).Normalize()

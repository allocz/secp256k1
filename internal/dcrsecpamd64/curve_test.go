...
func isValidJacobianPoint(point *JacobianPoint) bool {
	...
	// y^2/z^6 = x^3/z^6 + 7
	// y^2 = x^3 + 7*z^6
	var y2, z2, x3, result FieldVal64
...
func (p *JacobianPoint) Rescale(s *FieldVal64) {
	...
	// p.Y *= s^3
	// p.Z *= s
	sSquared := new(FieldVal64).SquareVal(s)
	sCubed := new(FieldVal64).Mul2(sSquared, s)
...
func TestDecompressYRandom(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		var oddY, evenY FieldVal64
		x := new(FieldVal64).Set(origX)
